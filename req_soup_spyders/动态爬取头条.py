...
class CrawlOptAnalysis(object):
    ...
    def _crawl_data(self, offset):
        ...
        """
        url = "http://www.toutiao.com/search_content/?offset={0}&format=json&keyword={1}&autoload=true&count=20&cur_tab=1".format(
            offset, urllib.parse.quote(self.search_word))
        print(url)
        try:
    ...
    def _save_picture(self, page_title, url):
        ...
        try:
            ...
            print("Image is saved! search_word={0}, page_title={1}, save_file={2}".format(self.search_word, page_title,
                                                                                          save_file))
        except Exception as e:
